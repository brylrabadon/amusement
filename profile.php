...
$user = require_login(); // any role
$pdo = db();
$flash = flash_get();

// Refresh user from DB so we have latest data
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'change_password') {
        ...
        if ($current === '' || $newPass === '' || $confirm === '') {
            flash_set('error', 'Please fill in all password fields.');
            redirect('profile.php');
        }
        if (strlen($newPass) < 8 ){
...
<div class="container" style="max-width:560px;">
  ...
  <div class="card" style="padding:1.5rem;margin-bottom:1.5rem;">
    <h2 style="font-size:1.25rem;font-weight:800;margin-bottom:1rem;">Account info</h2>
    <form method="post">
      ...
      <div class="form-group">
        <label>Email</label>
        <input type="email" value="<?= e($user['email'] ?? '') ?>" disabled style="background:#f1f5f9;color:#64748b;" />
        <small style="color:#94a3b8;font-size:.8rem;">Email cannot be changed here.</small>
      </div>
      <div class="form-group">
        <label>Phone</label>
      ...
      <div class="form-group">
        <label>Role</label>
        <div>
          <span class="badge <?= $isAdmin ? 'badge-yellow' : 'badge-blue' ?>"><?= e(ucfirst($user['role'] ?? 'customer')) ?></span>
        </div>
      </div>
  ...
  <div class="card" style="padding:1.5rem;">
    <h2 style="font-size:1.25rem;font-weight:800;margin-bottom:1rem;">Change password</h2>
    <form method="post">
