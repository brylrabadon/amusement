...
if ($date !== '') { $where[] = 'visit_date = ?';      $params[] = $date; }
...
$stats = ['total'=>0,'paid'=>0,'pending'=>0,'revenue'=>0.0];
try {
...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box}
    ...
    .bk-hero-inner{max-width:1200px;margin:0 auto;position:relative;z-index:1}
    .bk-hero h1{font-size:2rem;font-weight:900;color:#fff;margin:0 0 .35rem}
    .bk-hero p{color:rgba(255,255,255,.65);font-size:.95rem;margin:0}

    .bk-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;max-width:1200px;margin:-2.5rem auto 0;padding:0 2rem;position:relative;z-index:10}
    ...
    .bk-stat-value.green{color:#16a34a}.bk-stat-value.yellow{color:#d97706}.bk-stat-value.blue{color:#1e3a8a}

    .bk-body{max-width:1200px;margin:0 auto;padding:2rem}
    ...
    .filter-card input[type=text]{flex:1;min-width:200px}
    ...
    @media(max-width:768px){.bk-stats{grid-template-columns:1fr 1fr};.bk-body{padding:1.25rem 1rem}}
...
<div class="bk-stats">
  <div class="bk-stat"><div class="bk-stat-label">Total</div><div class="bk-stat-value blue"><?= $stats['total'] ?></div></div>
  ...
  <div class="bk-stat"><div class="bk-stat-label">Revenue</div><div class="bk-stat-value green">₱<?= number_format($stats['revenue'],0) ?></div></div>
</div>
...
<div class="bk-body">
  ...
  <div class="bk-table-card">
    <table>
      <thead>
        <tr><th>Reference</th><th>Customer</th><th>Ticket</th><th>Visit Date</th><th>Amount</th><th>Payment</th><th>Status</th><th>Tickets</th></tr>
      </thead>
      <tbody>
        ...
        <?php foreach($bookings as $b):
          $bId=$b['id'];$paySt=(string)($b['payment_status']??'');$bkSt=(string)($b['status']??'Active');
          $tNums=[];try{$ts=$pdo->prepare('SELECT ticket_number FROM tickets WHERE booking_id=? ORDER BY ticket_number');$ts->execute([$bId]);$tNums=$ts->fetchAll(\PDO::FETCH_COLUMN);}catch(\Throwable $e){}
          $pyC=['Paid'=>'badge-green','Pending'=>'badge-yellow','Cancelled'=>'badge-red','Refunded'=>'badge-blue'];
          $stC=['Active'=>'badge-green','Used'=>'badge-blue','Cancelled'=>'badge-red'];
        ?>
          <tr>
            <td><a href="../booking-detail.php?ref=<?= urlencode((string)($b['booking_reference']??'')) ?>" class="ref-pill"><?= e($b['booking_reference']??'') ?></a></td>
            <td><div style="font-weight:700"><?= e($b['customer_name']??'') ?></div><div style="color:#94a3b8;font-size:.75rem"><?= e($b['customer_email']??'') ?></div></td>
            <td><div style="font-weight:600"><?= e($b['ticket_type_name']??'') ?></div><div style="color:#64748b;font-size:.75rem">×<?= (int)($b['quantity']??1) ?></div></td>
            ...
            <td><span class="badge <?= $pyC[$paySt]??'badge-gray' ?>"><?= e($paySt) ?></span></td>
            <td><span class="badge <?= $stC[$bkSt]??'badge-gray' ?>"><?= e($bkSt) ?></span></td>
            <td><button onclick="toggleRow(<?= $bId ?>)" style="background:#eff6ff;border:1.5px solid #dbeafe;color:#1e3a8a;padding:.3rem .75rem;border-radius:.5rem;font-size:.78rem;font-weight:700;cursor:pointer">🎫 <?= count($tNums) ?></button></td>
          </tr>
          <tr class="ticket-expand-row" id="expand-<?= $bId ?>">
            <td colspan="8">
              ...
              <?php if(count($tNums)):foreach($tNums as $tn):?><span class="ticket-chip"><?= e($tn) ?></span><?php endforeach;else:?><span style="color:#94a3b8;font-size:.82rem">No tickets generated.</span><?php endif;?>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
  ...
  <div class="pagination" style="margin-top:1.5rem">
    ...
    <div class="pagination-pages">
      <?php
        $qs=http_build_query(array_filter(['q'=>$q,'pay'=>$pay,'date'=>$date]));
        $base='bookings.php?'.($qs?$qs.'&':'');
        echo '<a class="pg-btn '.($page<=1?'disabled':'').'" href="'.$base.'page='.($page-1).'">&#8592;</a>';
        ...
        echo '<a class="pg-btn '.($page>=$totalPages?'disabled':'').'" href="'.$base.'page='.($page+1).'">&#8594;</a>';
      ?>
    </div>
  </div>
  <?php endif; ?>
...
<script>function toggleRow(id){var r=document.getElementById('expand-'+id);if(r)r.style.display=r.style.display==='none'?'table-row':'none';}</script>
<?php render_footer(); ?>
</body>
</html>
